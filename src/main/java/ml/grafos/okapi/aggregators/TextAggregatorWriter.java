 ...
 * aggregator class.
 * 
 * We import most of the functionality from the Giraph implementation and modify
 * it to be more extensible.
 * 
 */
public class TextAggregatorWriter extends DefaultImmutableClassesGiraphConfigurable
		implements AggregatorWriter, MainAggregator {
	/** The filename of the outputfile */
	public static final String FILENAME = "textAggregatorWriter.filename";
	...
	/**
	 * The frequency of writing: - NEVER: never write, files aren't created at all -
	 * AT_THE_END: aggregators are written only when the computation is over - int:
	 * i.e. 1 is every superstep, 2 every two supersteps and so on
	 */
	public static final String FREQUENCY = "textAggregatorWriter.frequency";
	...
	@Override
	public void writeAggregator(Iterable<Entry<String, Writable>> aggregatorMap, long superstep) throws IOException {
	...
	/**
	 * Implements the way an aggregator is converted into a String. Override this if
	 * you want to implement your own text format.
	 *
	...
	protected String aggregatorToString(String aggregatorName, Writable value, long superstep) {
		return new StringBuilder("superstep=").append(superstep).append("\t").append(aggregatorName).append("=")
				.append(value).append("\n").toString();
	}
